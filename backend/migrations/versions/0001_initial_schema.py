...
def upgrade() -> None:
    # --- Enums ---
    # Use raw SQL with IF NOT EXISTS to safely handle cases where enum types
    # already exist in the database. sa.Enum.create(checkfirst=True) does not
    # work reliably with the asyncpg driver and can raise DuplicateObjectError.
    op.execute("CREATE TYPE IF NOT EXISTS userrole AS ENUM ('admin', 'manager', 'client', 'influencer')")
    op.execute("CREATE TYPE IF NOT EXISTS influencerstatus AS ENUM ('pending', 'active', 'inactive', 'suspended')")
    op.execute(
        "CREATE TYPE IF NOT EXISTS socialplatform AS ENUM ("
        "'instagram', 'tiktok', 'youtube', 'twitter', 'facebook', "
        "'pinterest', 'linkedin', 'snapchat', 'twitch')"
    )
    op.execute("CREATE TYPE IF NOT EXISTS clientstatus AS ENUM ('lead', 'active', 'paused', 'churned')")
    op.execute(
        "CREATE TYPE IF NOT EXISTS campaignstatus AS ENUM ("
        "'draft', 'planning', 'active', 'paused', 'completed', 'cancelled')"
    )
    op.execute(
        "CREATE TYPE IF NOT EXISTS campaigntype AS ENUM ("
        "'brand_awareness', 'product_launch', 'event_promotion', 'lead_generation', "
        "'app_install', 'sales', 'content_creation', 'affiliate')"
    )
    op.execute(
        "CREATE TYPE IF NOT EXISTS campaigninfluencerstatus AS ENUM ("
        "'invited', 'negotiating', 'contracted', 'content_due', 'content_submitted', "
        "'content_approved', 'published', 'completed', 'declined', 'dropped')"
    )
    op.execute(
        "CREATE TYPE IF NOT EXISTS deliverabletype AS ENUM ("
        "'instagram_post', 'instagram_story', 'instagram_reel', 'tiktok_video', "
        "'youtube_video', 'youtube_short', 'twitter_post', 'facebook_post', "
        "'blog_post', 'podcast_mention')"
    )
    op.execute(
        "CREATE TYPE IF NOT EXISTS deliverablestatus AS ENUM ("
        "'pending', 'in_progress', 'submitted', 'revision_requested', "
        "'approved', 'published', 'rejected')"
    )
    op.execute(
        "CREATE TYPE IF NOT EXISTS contractstatus AS ENUM ("
        "'draft', 'sent', 'viewed', 'signed_influencer', 'signed_agency', "
        "'fully_executed', 'voided', 'expired')"
    )
    op.execute(
    ...
    op.execute(
        "CREATE TYPE IF NOT EXISTS payoutstatus AS ENUM ("
    ...
    op.execute(
        ...
        "'campaign_completed', 'metrics_updated', 'system')"
    )
